...
let draw_shot_choosers (turn : player) (get_probabilities : position -> probabilities) (box : string list list) =
  let get_key = match turn with
  | One -> pos_to_p1_key
  | Two -> pos_to_p2_key
  in
  let draw_key (pos : position) (center_row : int) (center_col : int) (box : string list list) =
    let key_pic = pos |> get_key |> pic_of_key in
    let {miss; winner} = get_probabilities pos in
    let pic_with_probs : string =
      [
        key_pic;
        ["M"; string_of_int miss] |> String.concat "";
        ["W"; string_of_int winner] |> String.concat "";
      ] |> String.concat "\n" in
    let (startrow, startcol) = center_pic pic_with_probs center_row center_col in
    draw_pic {startrow; startcol; pic = pic_with_probs} box
  in
  box
...
let get_probabilities (st : state) (shot : position) =
  shot_probabilities {
    current_player_pos = if st.turn = One then st.p1_pos else st.p2_pos;
    opponent_pos = if st.turn = One then st.p2_pos else st.p1_pos;
    shot;
  }
...
let str_of_state (st : state) : string =
  [
    ...
    |> draw_shot_choosers st.turn (get_probabilities st)
    |> str_of_box;
    "\n";
    st.status;
  ] |> String.concat ""
